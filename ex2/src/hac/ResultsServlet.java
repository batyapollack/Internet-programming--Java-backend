...
public class ResultsServlet extends HttpServlet
{
    ...
    private void showResult(HttpServletRequest request, HttpServletResponse response) {
        ...
        ArraySavedResults voteResArr= (ArraySavedResults)getServletContext().getAttribute("ListOfFile");
        try {
            ...
            toClient.println("<form action = /ShowSurveyServlet method = post> <BR><BR><input type=\"submit\" name = btnSubmit value= Back to survey> </form> ");
            request.getRequestDispatcher("/ClosePage.html").include(request, response);
            toClient.close();
        }
     ...
     */
    private void buildHtml(PrintWriter toClient, ArraySavedResults voteResArr)
    {
     ...
     * @param voteResArr -  ArrayList that save all results of survey
     */
    private void checkCookies(HttpServletRequest request, PrintWriter toClient, HttpServletResponse response, ArraySavedResults voteResArr)
    {
        ...
        if (cookies != null)
        {
            for (Cookie value : cookies) {
                if (value.getName().compareTo("myCookie") == 0) {
                    firstvisit = false;
                    toClient.println("<div class=\"text-dark\"> <h6> You have already voted in the poll, you can vote only once </h6></div>");
                    break;
                }
            }
        }
    ...
    private Boolean fillSurvey(String item, HttpServletResponse response ) throws RedirectException {
        ...
        {
            try {
                response.sendRedirect("/ShowSurveyServlet");
            } catch (IOException e) {
